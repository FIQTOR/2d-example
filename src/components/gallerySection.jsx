...
import GalleryConfig from '../common/gallery';
...
const FirstPage = ({ handleNext, handleCloseSection, typewritingSoundEffect }) => {
  ...
  const text = GalleryConfig.fristPage
  ...
  return (
    <div className='absolute w-full min-h-screen bg-[rgba(238,99,160)] z-30 flex justify-center items-center backdrop-blur-sm'>
      <button className='absolute top-4 md:top-14 left-4 md:left-14 block z-10' onClick={() => handleCloseSection()}>
        <TbArrowBackUp className='text-white w-10 h-10' />
      </button>

      <img src="img/gallery/wave-top-left.png" alt="" className='absolute scale-150 md:scale-100 -top-20 -left-40  md:-top-[45vw] md:-left-60 unselectable' />
...
const SecondPage = ({ handleCloseSection, typewritingSoundEffect, clickSoundEffect }) => {
  const [isRendered, setIsRendered] = useState([true, true, true, true]); // Initialize an array for 4 items
  const [mailIndex, setMailIndex] = useState(null);
  const [displayText, setDisplayText] = useState('');
  const intervalRef = useRef(null);
  const [isFullTextDisplayed, setIsFullTextDisplayed] = useState(false);

  const text = GalleryConfig.galleryPage
  const gallery = GalleryConfig.gallery
  ...
  if (mailIndex !== null) {
    return <>
      <ShowPhoto mail={gallery[mailIndex]} mailIndex={mailIndex} handleMailIndex={handleMailIndex} handleRenderIndex={handleRenderIndex} typewritingSoundEffect={typewritingSoundEffect} />
    </>
  }

  return (
    <div className='absolute w-full min-h-screen bg-[rgba(238,99,160)] z-30 flex justify-center items-center backdrop-blur-sm overflow-hidden'>
      <button className='absolute top-4 md:top-14 left-4 md:left-14 ' onClick={() => handleCloseSection()}>
        <TbArrowBackUp className='text-white w-10 h-10' />
      </button>

      <img src="img/gallery/wave-top-left.png" alt="" className='absolute scale-150 md:scale-100 -top-20 -left-40  md:-top-[45vw] md:-left-60 unselectable -z-10  ' />
      ...
      <div className='w-full h-full flex flex-col md:flex-row justify-center items-center gap-8 md:gap-[10vw]'>
        <button onClick={stopTypewriterEffect} className='font-fredoka text-[#702c2b] md:pl-24'>
          ...
          <div src='' alt='' className='w-[17rem] md:w-[30rem] h-40 md:h-64 bg-[#f5a6c9] rounded-3xl p-2 md:p-4 flex items-center' >
        ...
        <div className='grid grid-cols-2 gap-y-4 md:gap-y-10 gap-x-4 md:gap-x-14 justify-center items-center scale-100 md:scale-75 max-w-3xl'>
          ...
          <button className={`${isRendered[0] ? 'scale-100' : 'scale-150'} relative w-40 h-40 md:w-full md:h-auto duration-300`} onClick={() => handleMailIndex(0)}>
            ...
            <div className={`${isRendered[0] ? 'translate-x-0 scale-100' : 'translate-x-16 md:-translate-x-[40rem] -translate-y-20 md:translate-y-40 scale-[2.0]'}w-full h-full rounded-xl md:rounded-3xl overflow-hidden duration-300`}>
              <img src={gallery[0]?.image} alt="" className='-z-10' />
            </div>
          </button>
          <button className={`${isRendered[1] ? 'scale-100' : 'scale-150'} relative w-40 h-40 md:w-full md:h-auto duration-300`} onClick={() => handleMailIndex(1)}>
            ...
            <div className={`${isRendered[1] ? 'translate-x-0 scale-100' : '-translate-x-16 md:-translate-x-[40rem] -translate-y-20 md:translate-y-40 scale-[2.0]'}w-full h-full rounded-xl md:rounded-3xl overflow-hidden duration-300`}>
              <img src={gallery[1]?.image} alt="" className='-z-10' />
            </div>
          </button>
          <button className={`${isRendered[2] ? 'scale-100' : 'scale-150'} relative  w-40 h-40 md:w-full md:h-auto duration-300`} onClick={() => handleMailIndex(2)}>
            ...
            <div className={`${isRendered[2] ? 'translate-x-0 scale-100' : 'translate-x-16 md:-translate-x-[40rem] -translate-y-40 scale-[2.0]'}w-full h-full rounded-xl md:rounded-3xl overflow-hidden duration-300`}>
              <img src={gallery[2]?.image} alt="" className='-z-10' />
            </div>
          </button>
          <button className={`${isRendered[3] ? 'scale-100' : 'scale-150'} relative  w-40 h-40 md:w-full md:h-auto duration-300`} onClick={() => handleMailIndex(3)}>
            ...
            <div className={`${isRendered[3] ? 'translate-x-0 scale-100' : '-translate-x-16 md:-translate-x-[40rem] -translate-y-40 scale-[2.0]'}w-full h-full rounded-xl md:rounded-3xl overflow-hidden duration-300`}>
              <img src={gallery[3]?.image} alt="" className='-z-10' />
            </div>
          </button>
          {/* End Gallery */}
        </div>
...
const ShowPhoto = ({ mail, handleMailIndex, handleRenderIndex, mailIndex, typewritingSoundEffect }) => {
  ...
  return (
    <div className='absolute bg-[rgba(238,99,160)] flex flex-col md:flex-row justify-center items-center w-full min-h-screen z-30 gap-10'>
      <button className='absolute top-4 md:top-14 left-4 md:left-14' onClick={() => { handleMailIndex(null); handleRenderIndex(mailIndex); }}>
        <TbArrowBackUp className='text-white w-10 h-10' />
      </button>
